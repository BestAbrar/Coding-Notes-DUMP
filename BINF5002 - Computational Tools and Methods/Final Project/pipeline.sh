...
CLEANED_DIR=${RESULTS_FOLDER}/cleaned
...
mkdir -p $RAW_DIR $ALIGNED_DIR $VARIANT_DIR $ANNOTATED_DIR $QC_DIR $SNPEFF_DATA_DIR $CLEANED_DIR
...
echo Cleaning Disease Sequence
fastp -i $RAW_DIR/${SRA}.fastq -o $CLEANED_DIR/${SRA}_fastp.fastq -h $QC_DIR/${SRA}fastp.html -h $QC_DIR/${SRA}fastp.json
...
bwa mem -R '@RG\tID:1\tLB:lib1\tPL:illumina\tPU:unit1\tSM:sample1' $RAW_DIR/reference.fasta $CLEANED_DIR/${SRA}_fastp.fastq > $ALIGNED_DIR/aligned.sam
echo Aligned reads!
...
snpEff -c $SNPEFF_DIR/snpEff.config -stats $SNPEFF_DIR/snpEff.html reference_db $VARIANT_DIR/filtered_variants.vcf > $VARIANT_DIR/annotated_variants.vcf
